...
@Author: Gemini & User
@Date: 2025-10-14
@Version: 16.8 (Default Delete Source)
@Description:
    一个将TXT文本文件转换为EPUB电子书的自动化脚本。
    此版本根据用户要求，将“就地整合”模式下的源文件删除功能默认为开启。
"""
...
# 默认值已修改为 True (自动删除)
DELETE_SOURCE_ON_MERGE = True # <--- 默认值已修改为 True
...
# ... 后续所有代码 (TextParser, EbookGenerator, main 函数等) 与 v16.7 完全相同，保持不变 ...
...
def find_common_book_name(file_list, source_dir):
    ...
    common_prefix = os.path.commonprefix(file_list); cleaned_name = common_prefix.strip(' _-').strip()
    if len(cleaned_name) < 2: folder_name = os.path.basename(source_dir); logging.warning(f"共同文件名前缀 '{cleaned_name}' 过短或为空，将使用文件夹名称 '{folder_name}' 作为书名。"); return folder_name
    logging.info(f"从文件名中提取共同书名: '{cleaned_name}'"); return cleaned_name
def read_file_with_fallback(file_path):
    encodings_to_try = ['utf-8', 'utf-16', 'gbk']
    for enc in encodings_to_try:
        try:
            with open(file_path, 'r', encoding=enc) as f: content = f.read()
            logging.info(f"文件编码识别为: {enc.upper()} ({os.path.basename(file_path)})"); return content
        except (UnicodeDecodeError, UnicodeError): logging.debug(f"尝试 {enc.upper()} 编码失败...")
    logging.error(f"读取文件失败: {file_path}，尝试 {encodings_to_try} 编码后均失败。"); return None
class TextParser:
    def parse_chapters(self, content, force_sort=False):
        ...
        for i, match in enumerate(matches):
            ...
            volume_match = VOLUME_REGEX.search(title); chapter_match = CHAPTER_REGEX.search(title)
            vol_num, chap_num = current_volume, float('inf')
            if volume_match:
                try: current_volume = cn2an.cn2an(volume_match.group(1), "smart"); vol_num = current_volume; chap_num = 0
                except Exception: logging.warning(f"无法转换卷号: {title}")
            if chapter_match:
                try: chap_num = cn2an.cn2an(chapter_match.group(1), "smart")
                except Exception: logging.warning(f"无法转换章节号: {title}")
        ...
        if ENABLE_SORTING or force_sort:
            logging.info("正在进行分层排序..."); sorted_chapters = sorted(deduplicated_chapters, key=lambda x: x['sort_key']); logging.info("章节排序完成。"); return sorted_chapters
        else:
            logging.info("排序已关闭。"); return deduplicated_chapters
class EbookGenerator:
    def __init__(self, author="Unknown Author"): self.author = author
    def create_epub(self, dest_path, book_title, chapters, full_content):
        book = epub.EpubBook(); book.set_identifier(book_title); book.set_title(book_title); book.set_language('zh'); book.add_author(self.author)
        if not chapters:
            logging.warning("未识别到任何有效章节..."); html_content = full_content.replace('\n', '<br />'); chapter = epub.EpubHtml(title=book_title, file_name='chap_1.xhtml', lang='zh'); chapter.content = f'<h1>{book_title}</h1><p>{html_content}</p>'; book.add_item(chapter); book.toc = [epub.Link('chap_1.xhtml', book_title, 'intro')]; book.spine = ['nav', chapter]
        else:
            logging.info(f"按顺序开始处理 {len(chapters)} 个章节..."); epub_chapters = []
            for i, chap_data in enumerate(chapters):
                chap_title, chap_content = chap_data['title'], chap_data['content']; logging.info(f"    - (顺序 {i+1}) 章节标题: {chap_title}"); preview = chap_content[:40].replace('\n', ' ').replace('\r', ' '); logging.info(f"      章节内容预览: {preview}..."); file_name = f'chap_{i+1}.xhtml'; epub_chap = epub.EpubHtml(title=chap_title, file_name=file_name, lang='zh'); html_content = chap_content.replace('\n', '<br />'); epub_chap.content = f'<h1>{chap_title}</h1><p>{html_content}</p>'; book.add_item(epub_chap); epub_chapters.append(epub_chap)
            book.toc = [epub.Link(c.file_name, c.title, f'chap_{i+1}') for i, c in enumerate(epub_chapters)]; book.spine = ['nav'] + epub_chapters
        book.add_item(epub.EpubNcx()); book.add_item(epub.EpubNav())
        try:
            epub.write_epub(dest_path, book, {}); logging.info(f"成功转换: {dest_path}")
...
def send_bark_notification(title, body):
    ...
    try:
        url = f"{bark_url.rstrip('/')}/{title}/{body}?icon=https://raw.githubusercontent.com/yueshang/pic/main/miao/15.jpg"; response = requests.get(url, timeout=10)
        if response.status_code == 200: logging.info("Bark 通知发送成功。")
        else: logging.warning(f"Bark 通知发送失败: {response.status_code}")
    except Exception as e: logging.error(f"发送 Bark 通知时发生网络错误: {e}")
...
def process_merged_files(source_dir, file_list, master_txt_path, dest_epub_path, parser, generator):
...
def main():
    logging.info("================== 开始执行TXT转EPUB任务 (v16.8) ==================");
    ...
    if not os.path.isdir(DESTINATION_FOLDER):
        logging.info(f"目标文件夹不存在，正在创建..."); os.makedirs(DESTINATION_FOLDER, exist_ok=True)
    ...
    for dirpath, _, filenames in os.walk(SOURCE_FOLDER):
        ...
        if ENABLE_MERGE_MODE and len(txt_files) > 1 and not is_root_dir:
        ...
        try:
            if task['type'] == 'merge':
                ...
                if os.path.exists(dest_epub_path) or os.path.exists(master_txt_path):
                    logging.info(f"合并任务 '{source_dir}' 对应的EPUB或主TXT已存在，跳过（就地整合模式下不重复处理源）。")
                    continue
